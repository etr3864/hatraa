...
## חוק ברזל: חתימה וכותרת
- תחילת המכתב חייבת להתחיל בשם העו"ד שסופק (למשל "עו"ד משה שועלי"), לא בשם הלקוח
- כתובת / טלפון / מייל בכותרת = של העו"ד בלבד
- סיום המכתב: "בכבוד רב," ואז בשורה הבאה שם העו"ד בלבד
- אסור לחתום בשם הלקוח. הלקוח מוזכר רק כ"מרשי" / "מרשיי"
- דוגמת סיום נכונה:
בכבוד רב,
עו"ד משה שועלי
...
export async function rewriteAsAttorney(
  ...
  const userPrompt = `פרטי העו"ד (השולח והחותם היחיד במכתב המשוכתב):
${buildAttorneyBlock()}

הלקוח המיוצג (מרשי בלבד — אסור לחתום בשמו): ${clientLabel}
נמען: ${sanitizeInput(input.respondentName)}

חובה:
- כותרת הפתיחה: ${ATTORNEY.displayName}
- חתימה בסיום: "בכבוד רב," ואז ${ATTORNEY.displayName}
- אסור לכתוב את שם הלקוח (${clientLabel}) כחותם או כשולח בכותרת
